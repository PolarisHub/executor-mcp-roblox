...
export default defineTool({
  name: "find-remote-xrefs",
  title: "Find functions that fire/handle a remote (IDA xrefs)",
  description:
    "Resolve a RemoteEvent / RemoteFunction / BindableEvent / BindableFunction from a Luau expression, then walk " +
    "every function in the GC and report which ones reference it — the runtime equivalent of cross-referencing a " +
    "network endpoint. A referrer references the remote if it appears in the function's upvalues OR constants, which " +
    "surfaces the closures that FireServer/InvokeServer/OnClientEvent-connect this remote. Each xref reports " +
    '__fnInfo (ptr/name/source/line/nparams/nups) plus \'via\' ("upvalue" or "constant"); the response includes ' +
    "the remote's ClassName. Requires getgc + getupvalues/getconstants; caps the scan and flags truncation.",
  category: "Disassembly & Xrefs",
  input: z.object({
    remotePath: z
      .string()
      .describe(
        "Luau expression resolving to a RemoteEvent/RemoteFunction/Bindable, e.g. " +
          "'game.ReplicatedStorage.Remotes.AttackEvent' or 'getrenv().game.ReplicatedStorage.Net.RF'. " +
  ...
  async execute({ remotePath, limit, maxScan, threadContext }, ctx) {
